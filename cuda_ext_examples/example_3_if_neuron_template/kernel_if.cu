...
std::vector<at::Tensor> if_tbn_forward(const at::Tensor x, float v_th) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(), "if_tbn_forward_kernel", ([&] {
        if_tbn_forward_kernel<scalar_t><<<numel / 256 + 1, 256>>>(
...
std::vector<at::Tensor> if_tbn_backward(const at::Tensor grad_y, const at::Tensor m, const at::Tensor y,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_y.scalar_type(), "if_tbn_forward_kernel", ([&] {
        if_tbn_backward_kernel<scalar_t><<<numel / 256 + 1, 256>>>(
                T, BN, grad_y.data_ptr<scalar_t>(), y.data_ptr<scalar_t>(), m.data_ptr<scalar_t>(), v_th, alpha,
