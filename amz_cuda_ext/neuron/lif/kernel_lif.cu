...
__global__ void lif_forward_kernel(int numel, int T, int N, const float* x, const float tau, const float v_th,
...
__global__ void lif_backward_kernel(int numel, int T, int N, const float* grad_y, const float* y, const float* m,
...
std::vector<at::Tensor> forward_lif(const at::Tensor x, float tau, float v_th, float alpha) {
    ...
    lif_forward_kernel<<<numel / 256 + 1, 256>>>(numel, T, N, x.data_ptr<float>(),
                                                 tau, v_th, alpha, y.data_ptr<float>(), m.data_ptr<float>());
...
std::vector<at::Tensor> backward_lif(const at::Tensor grad_y, const at::Tensor m, const at::Tensor y,
    ...
    lif_backward_kernel<<<numel / 256 + 1, 256>>>(numel, T, N, grad_y.data_ptr<float>(), y.data_ptr<float>(),
