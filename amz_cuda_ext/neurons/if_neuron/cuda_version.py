...
class if_tbn(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        m_seq, y_seq = if_tbn_forward(x_seq, v_th, alpha)
...
class if_btn(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        x_seq, v_th, alpha = args
        m_seq, y_seq = if_btn_forward(x_seq, v_th, alpha)
        if x_seq.requires_grad:
            ctx.save_for_backward(m_seq, y_seq)
            ctx.v_th = v_th
            ctx.alpha = alpha
...
class IF_TBN(nn.Module):
    def __init__(self, thresh=1.0, alpha=1.0):
        super(IF_TBN, self).__init__()
        self.if_tbn = if_tbn.apply
        self.v_th = thresh
        self.alpha = alpha

    def forward(self, x):
        y = self.if_tbn(x, self.v_th, self.alpha) * 1.0
        return y


class IF_BTN(nn.Module):
    def __init__(self, thresh=1.0, alpha=1.0):
        super(IF_BTN, self).__init__()
        self.if_btn = if_btn.apply
        self.v_th = thresh
        self.alpha = alpha

    def forward(self, x):
        y = self.if_btn(x, self.v_th, self.alpha) * 1.0
        return y
